...
int get_bitmap ()
{
	unsigned int bg; /* Block Group */

	printf ("Loading allocation bitmap...    "); fflush (stdout);

	status[0] = BOOTBLOCK;
	for (bg = 0; bg < block_groups_count; bg++) {
		unsigned int bm_off; /* block BitMap OFFset */
		unsigned int block  = BASE_BLOCK (bg);
		unsigned int last =
			(block + 8 * blocksize > superblock.s_blocks_count)
			? superblock.s_blocks_count : block + 8 * blocksize;

		fs_read_block (group_descriptors[bg].bg_block_bitmap, buffer);

		for (bm_off = 0; block < last; bm_off++) {
			unsigned int bit;

			for (bit = 0; bit < 8 && block < last; bit++, block++)
			{
				if (buffer[bm_off] & (1 << bit) )
...
{
	unsigned int i, j;
	unsigned int max_inode = (superblock.s_inodes_per_group
		* superblock.s_inode_size - 1) / blocksize + 1;

	printf (" filesystem structures..."); fflush (stdout);

	for (i = 0; i < block_groups_count; i++) {
		if (group_descriptors[i].bg_block_bitmap != BASE_BLOCK (i) )
		{
			int j;

			status[BASE_BLOCK (i)] = SUPERBLOCK;

			for (j = 0; j < group_descriptor_table_size; j++) {
				status[BASE_BLOCK (i) + j + 1] = GRP_DESC_TBL;
			}
		}

		status[group_descriptors[i].bg_block_bitmap] = BLOCK_BMP;
		status[group_descriptors[i].bg_inode_bitmap] = INODE_BMP;

		for (j = 0; j < max_inode; j++) {
			status[group_descriptors[i].bg_inode_table + j]
				= INODE_TBL;
		}
	}
